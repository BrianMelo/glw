...
#'      
#' @export
mode_scr <- function(Data, prior, mixture) {
  if(nchar(mixture)>1) out <- mode_scr_mix(Data=Data, prior=prior, mixture=mixture)
  if(nchar(mixture)==1) out <- mode_scr_single(Data=Data, prior=prior, mixture=mixture)
  return(out)
}
...
mode_scr_mix <- function(Data, prior, mixture){
...
mode_scr_single <- function(Data, prior, mixture){
  Model <- function (parm, Data, prior, mixture) {
